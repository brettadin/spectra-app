...
## Verification
- `pytest tests/server/test_nist_quant_ir.py -q` 【857d27†L1-L3】
...
## Tasking — Provide Quant IR fallbacks for water, methane, and CO2
- Ensure the preset Quant IR selector can fetch 0.125&nbsp;cm⁻¹ overlays for H₂O, methane, and CO₂ even though the catalog omits them.

## Actions & Decisions
- Registered manual WebBook mappings for H2O, methane, and CO2, merged them into the cached catalog, and exposed provenance flags so the selector resolves the new tokens. 【F:app/server/fetchers/nist_quant_ir.py†L151-L187】【F:app/server/fetchers/nist_quant_ir.py†L412-L463】
- Parsed JCAMP Δx metadata, resampled manual payloads onto the 0.125&nbsp;cm⁻¹ grid, and recorded the source sampling rate in metadata/provenance. 【F:app/server/fetchers/nist_quant_ir.py†L238-L395】
- Surface manual provenance guidance in the Quant IR sidebar and documented the workflow in the quick-start guide. 【F:app/ui/main.py†L1579-L1599】【F:docs/app/user_guide.md†L15-L23】

## Verification
- `pytest tests/server/test_nist_quant_ir.py -q` 【02cca5†L1-L3】

## Docs Consulted
- `docs/app/user_guide.md` 【F:docs/app/user_guide.md†L15-L23】
...
## Tasking — Restore overlay workspace table visibility controls
- Prevent the overlay visibility editor from crashing after Quant IR downloads introduce traces lacking a `points` attribute.

## Actions & Decisions
- Exposed a derived `points` property on overlay traces so the workspace table can display Quant IR overlays without raising `AttributeError`. 【F:app/ui/main.py†L117-L138】【F:app/ui/main.py†L2332-L2344】
- Guarded image overlays to report zero points when their payload is missing or unparseable, avoiding misleading counts. 【F:app/ui/main.py†L117-L138】
- Updated release collateral with the v1.2.1p patch notes, version metadata, and patch log entry. 【F:app/version.json†L1-L5】【F:docs/patch_notes/v1.2.1p.md†L1-L11】【F:PATCHLOG.txt†L33-L41】
