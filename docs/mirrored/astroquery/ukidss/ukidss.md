...
If you want to change your programme\_id and database after you have already instantiated the
...
can do so by using [`list_catalogs()`](../api/astroquery.ukidss.UkidssClass.html#astroquery.ukidss.UkidssClass.list_catalogs "astroquery.ukidss.UkidssClass.list_catalogs"):
...
[`astropy.coordinates`](https://docs.astropy.org/en/stable/coordinates/ref_api.html#module-astropy.coordinates "(in Astropy v7.1)"). The images are returned as a list of
[`HDUList`](https://docs.astropy.org/en/stable/io/fits/api/hdulists.html#astropy.io.fits.HDUList "(in Astropy v7.1)") objects.
...
that you already have a [`UkidssClass`](../api/astroquery.ukidss.UkidssClass.html#astroquery.ukidss.UkidssClass "astroquery.ukidss.UkidssClass") object `u_obj` instantiated:
...
desired value, you should enter it using a [`Quantity`](https://docs.astropy.org/en/stable/api/astropy.units.Quantity.html#astropy.units.Quantity "(in Astropy v7.1)") object
with appropriate units or as a string that is parsable by
[`Angle`](https://docs.astropy.org/en/stable/api/astropy.coordinates.Angle.html#astropy.coordinates.Angle "(in Astropy v7.1)"). Another parameter you may set is
...
[`get_images()`](../api/astroquery.ukidss.UkidssClass.html#astroquery.ukidss.UkidssClass.get_images "astroquery.ukidss.UkidssClass.get_images") by a call to
[`get_image_list()`](../api/astroquery.ukidss.UkidssClass.html#astroquery.ukidss.UkidssClass.get_image_list "astroquery.ukidss.UkidssClass.get_image_list") with exactly the same
...
cases by using a [`Quantity`](https://docs.astropy.org/en/stable/api/astropy.units.Quantity.html#astropy.units.Quantity "(in Astropy v7.1)") or a string parsable via
[`Angle`](https://docs.astropy.org/en/stable/api/astropy.coordinates.Angle.html#astropy.coordinates.Angle "(in Astropy v7.1)"). If this is missing, then it defaults to 1 arcmin.
As before you may also mention the `programme_id` and the `database`. The query
results are returned in a [`Table`](https://docs.astropy.org/en/stable/api/astropy.table.Table.html#astropy.table.Table "(in Astropy v7.1)").
